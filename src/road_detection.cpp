...
int main( int argc, char **argv ) {
    ...
    namedWindow(edgemap_window_name, cv::WINDOW_NORMAL);
    namedWindow(lines_window_name, cv::WINDOW_NORMAL);
    ...
    Canny(blurred, dst, lowThreshold, lowThreshold*ratio_, kernel_size);
    if (!dst.empty()) {
        cout << dst.channels();
        ...
        line( cdst, Point(mid_x, cdst.size().height), Point(mid_x + (avg_slope*cdst.size().height), 0), Scalar(255, 0, 0), 3, LINE_AA);
        //show the image
